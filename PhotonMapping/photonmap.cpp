...
void  PhotonMap::LocatePhotons(KDTreeNode* t, Vector3 pos, float maxDist2, std::vector<Photon>& result)
{
	...
	{
		float distToPos = pos.Distance2(t->photon.pos);
...
Color PhotonMap::GetIrradiance(Vector3& pos, Vector3& normal, float maxDist, int maxUse)
{
	Color ret;
	...
	for (int i = 0; i <maxUse&&i<result.size(); ++i)
	{
	...
	ret *= (1.f/(PI*maxDist*maxDist*emitPhotons));
